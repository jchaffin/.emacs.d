...
- [[#base-configuration][Base Configuration]]
  ...
  - [[#navigation][Navigation]]
    - [[#bookmark][Bookmark]]
  - [[#editing][Editing]]
    - [[#mark-forward-sexp][Mark Forward Sexp]]
...
#+BEGIN_SRC emacs-lisp
(defun mplist-get-values (plist prop)
  "Get the values asssociated to PROP in PLIST, a modifeid plist.

A modified plist is one where keys are keywords and values
are non-keywords elements that follow it.

If there are multiple properties with the same keyword,
only the first property and its values is returned."
  (let ((tail plist)
        result)
    ...
    (while (and (consp tail) (not (keywordp (car tail))))
      (push (popo tail) result))
    (nreverse result)))

(defun mplist-get-value (plist prop)
  "Get a single value associated to PROP in PLIST, a modified plist."
  (car (mplist-get-values plist prop)))

(defun mplist-remove (plist prop)
  "Return a copy of a modified PLIST without PROP and its values.

If there are multiple properties with the same keyuword, only the first property
...
*** Encryption
:PROPERTIES:
:ID:       F871883A-D25D-4045-9A13-34FA7707768B
:END:
**** GnuTLS
...
:PROPERTIES:
:ID:       51BDC796-E9D9-4B1D-A081-487C626CC192
:END:
**** esup
...
*** Straight.el
:PROPERTIES:
:ID:       00484055-B339-4BAA-93EA-2693695336C3
:END:
...
*** Reload/Eval
:PROPERTIES:
:ID:       B88518E2-0B32-4706-B210-645D7889646E
:END:
**** Reload Init
...
(use-package hydra
  ...
  "
      ...
      ("q" nil)))
  :bind
  (:prefix-map emacs-utility-prefix-map
               :prefix-docstring "Emacs utility map"
               :prefix "C-c e"
               ("r" . hydra-reload-eval/body)
               ("p"  . hydra-straight/body))
  :config
...
*** Bookmark
:PROPERTIES:
:ID:       41010DDA-09D3-4A6F-9C49-8DE32F61DAFB
:END:
**** Bookmark File
...
***** Adaptive Wrap
:PROPERTIES:
:ID:       FF2121FE-1369-4B53-B9AE-B150E612E556
:END:
...
*** Spellchecking
:PROPERTIES:
:ID:       9BC08A6F-9589-4A2E-B2AD-9E1E1BA8DC9B
:END:
...
*** System
:PROPERTIES:
:ID:       8AA04A03-032A-4FC8-88B1-F712F6235A6D
:END:
**** Use Emacs terminfo
...
** Buffer
:PROPERTIES:
:ID:       8A78FBDC-E7F7-4119-8755-D34B8109F7F8
:END:
...
*** Frame Startup
:PROPERTIES:
:ID:       2F513F76-AF1A-4536-B715-A6F648B7AF38
:END:
**** Display Time
:PROPERTIES:
:ID:       15A7EBBE-44FB-4F7F-B2CC-7C8FDE939BDA
:END:
...
**** Transpose Frame
:PROPERTIES:
:ID:       2F4F7F68-87C1-4BD2-8C19-247751BE2E19
:END:
...
**** posframe
:PROPERTIES:
:ID:       34EA4A31-ADF2-4B0C-811A-1DCA53AE8789
:END:
...
** emms
:PROPERTIES:
:ID:       56DD666B-7EF3-495B-B45C-F617B5D1BBD6
:END:
...
(use-package company-tabnine
  ...
  (defun halidom/company--transform-candidates (func &rest args)
    (if (not company-tabnine--disable-next-transform)
      (apply func args)
    (setq company-tabnine--disable-next-transform nil)
    (car args)))

  (defun halidom/company-tabnine (func &rest args)
    (when (eq (car args) 'candidates)
      (setq company-tabnine--disable-next-transform t))
    (apply func args))

  (advice-add #'company--transform-candidates
              :around #'halidom/company--transform-candidates)
  (advice-add #'company-tabnine :around #'halidom/company-tabnine))
#+END_SRC
...
*** Skeletor
:PROPERTIES:
:ID:       9D7AE17E-5FB7-421F-A92F-C3E43BD26B14
:END:
...
** Text Completion
:PROPERTIES:
:ID:       ED959BFE-949F-43C4-BE02-4BC01D43FC05
:END:
*** Company Dict
...
(use-package xwidget
  ...
  :init
  (defadvice xwidget-webkit-browse-url (url )
...
(use-package hideshow-org
  ...
  (defun hideshow-org/enable ()
    (hs-org/minor-mode +1))

  :hook
  (org-mode . hideshow-org/enable))
#+END_SRC
