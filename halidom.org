...
- [[#base-configuration][Base Configuration]]
  ...
  - [[#core][Core]]
    ...
    - [[#files][Files]]
    - [[#modernize-emacs][Modernize Emacs]]
    - [[#global-autorevert][Global Autorevert]]
    - [[#cleanup-whitespace][Cleanup Whitespace]]
    - [[#prefer-newer-bytecode][Prefer newer bytecode]]
    - [[#better-beginning-of-line][Better Beginning of Line]]
    - [[#alias-yes-or-no][Alias Yes Or No]]
    - [[#encryption][Encryption]]
    - [[#text-editing][Text Editing]]
    - [[#profiling-and-rebooting][Profiling and Rebooting]]
...
- [[#customizing-emacs][Customizing Emacs]]
  - [[#project-management][Project Management]]
    ...
    - [[#projectile-codesearch][Projectile Codesearch]]
    - [[#projectile-ripgrep][Projectile Ripgrep]]
    - [[#projectile-treemacs][Projectile Treemacs]]
    - [[#skeletor][Skeletor]]
...
- [[#user-interface][User Interface]]
  ...
  - [[#fonts][Fonts]]
    - [[#modify-glyph-display][Modify Glyph Display]]
...
;; Unadvise
<<utility-unadvise>>
;; shell command to string trim final newline
<<utlity-shell-trim>>
#+END_SRC
...
(defalias #'straight-repos-dir (apply-partially #'straight-dir "repos"))
(defalias #'straight-build-dir (apply-partially #'straight-dir "build"))
...
**** Mark Forward Sexp
:PROPERTIES:
:ID:       38FC590F-CB08-44C6-BAC8-A088BDBEFD03
:END:
...
(use-package hydra
  :bind*
  ("C-c e" . hydra-reload-eval/body)
  ("C-c s" . hydra-straight/body)
  :init
  (eval-and-compile
...
(use-package general
  ...
  (general-create-definer halidom-leader
    :prefix "M-m"
    :prefix-command 'halidom-prefix-command
    :prefix-map 'halidom-prefix-map)

  (setq macos-apps-icon
        (concat (propertize " " 'display '(raise -0.20)) "Apps"))

  (general-override-mode)
  (halidom-leader
    "" '(nil :which-key " Halidom Command Center")
    "h" '(:ignore t :wk " Help")
    "m" '(:ignore t :wk " macOS")
    "mt" '(:keymap macos-iterm-command-map
                   :wk " iTerm"
                   :package macos-dev-utils)
    "mo" '(:keymap macos-open-with-command-map
                   :wk " Editors"
                   :package macos-dev-utils)
    "ma" `(counsel-osx-app :wk ,macos-apps-icon)
    "s" '(hydra-straight/body :wk " Straight")
    "r" '(hydra-reload-eval/body :wk " Reload/Eval")))
#+END_SRC
  ...
  (defun system-packages/update-brew-commands (commands)
    ...
    (let ((brew-commands-alist
      ...
      (dolist (command commands)
        (destructuring-bind (cmd . cmd-string) command
...
;; (which-key-add-key-based-replacements "M-m m" " MacOS")
#+END_SRC
...
(use-package macos-dev-utils
  ...
  ;; (halidom-leader
  ;;   ;; Iterm Command Map
  ;;   "mtf" '(iterm-focus :wk " Focus")
  ;;   "mtg" '(iterm-goto-filedir-or-home :wk " File or Directory")
  ;;   "mts" '(iterm-shell-command :wk " Shell Command")
  ;;   ;; Editor Command Map
  ;;   "moo" '(macos-open-with-default-app :wk " Default App")
  ;;   "mos" '(open-with-sublime-text :wk " Sublime Text 3")
  ;;   "moa" '(open-with-atom :wk " Atom")
  ;;   "mov" '(open-with-vscode :wk " VS Code")
  ;;   "mob" '(open-with-bbedit :wk " BBEdit")
  ;;   "moc" '(open-with-coda :wk " Coda")
  ;;   "mot" '(open-with-tower :wk " Tower"))
  )
...
(use-package osx-dictionary
  ...
  (progn
    ...
    ;; (set-fontset-font "fontset-default" '#xE8CE
    ;;                   "Material Icons" nil 'prepend)
    ;; (which-key-add-key-based-replacements "M-m m d" " Dictionary")
...
(use-package reftex
    ...
    (defun halidom/latex-reftex-hook ()
      ...
      (setq reftex-plug-into-AUCTeX '(nil nil t t t)
            reftex-use-fonts t
            reftex-default-bibliography (list (file-truename "~/Dropbox/org/ref/references.bib"))))
    :hook
...
(add-hook 'org-mode-hook #'org-visual-fill-column)
...
(add-to-list 'org-capture-templates
  '("t" "Todo" entry (file+headline "~/Dropbox/todos/TODOs.org" "Tasks")))
...
#+NAME: org-protocol-app
#+BEGIN_SRC emacs-lisp :tangle no
(setq org-protocol-default-template-key "l")
(add-to-list 'org-capture-templates
             '("l" "Link" entry
               (file+olp "~/Dropbox/org/capture.org" "Web Links")
               "* %:description\n%u\n\n%c\n\n%i"))
...
(use-package org-mac-protocol
  ...
  (defun org-mac-protocol-install (&optional force)
    ...
    (interactive "P")
    (let* ((target-directory "~/Library/Scripts/")
...
#+BEGIN_SRC emacs-lisp :tangle no
  (use-package org-journal
    ...
    ;; :general
    ;; (halidom-leader
	  ;;   "ojn" '(org-journal-new-entry :wk "new entry")
    ;;   "ojs" '(org-journal-search :wk "search")

    ;;   "ojS" '(org-journal-search-forever :wk "search forever"))
    :init
...
(use-package org-sidebar
  ...
  (course-sidebar "ling165b" "Ling 165B Syntax"
                   (:name " Paper" :tag "paper"))

  (course-sidebar "cs131" "Programming Languages"
                  (:name " Upcoming" :anything))
  (course-sidebar "ling103" "Phonetics" (:name " Project" :tag "project"))

  (cl-defun org-course-sidebar ()
    (interactive)
    (let ((todo-file (car (directory-files
  ...
  (add-hook 'projectile-after-switch-project-hook #'org-course-sidebar))
#+END_SRC
...
(use-package org-edit-latex
  :custom
  (org-edit-latex-frag-master (emacs-etc-dir "preview" "frag-master.tex")))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :tangle no
  (use-package org-ref
    ...
    (org-ref-default-bibliography
     '("~/Dropbox/org/ref/references.bib"))
    (org-ref-bibliography-notes
     "~/Dropbox/org/ref/refnotes.org")
    (org-ref-notes-directory
     "~/Dropbox/org/notebooks")
    (org-ref-pdf-directory
     "~/Dropbox/Documents/pdfs/")
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (use-package visual-regexp
    ...
    (which-key-add-key-based-replacements
      "C-c r" "Visual RegExp"
      "C-c r r" "replace"
      "C-c r q" "query replace"
      "C-c r m" "multiple cursors"))
#+END_SRC
...
#+NAME: ui/fonts
#+BEGIN_SRC emacs-lisp :tangle no :noweb yes :exports none
;; change dislay of certain glyph codes
<<fonts/glyphs>>
;; font variables
<<fonts/variables>>
...
;; snippets
<<completion-snippets>> ;; end snippets
;; minibuffer
<<completion-minibuffer>> ;; end minibuffer
#+END_SRC
...
<<candidates/company>>
;; prescient
<<candidates/prescient>>
#+END_SRC

*** Company

#+NAME: candidates/company
#+BEGIN_SRC emacs-lisp :tangle no :noweb yes :exports none
;; company mode
<<company/mode>>
;; TabNine
...
(use-package company-tabnine
  ...
  :demand t
  :custom
  ;; set the binaries folder
  (company-tabnine-binaries-folder (emacs-var-dir "company" "tabnine"))
  :init
  (add-to-list 'company-backends #'company-tabnine)
  :config
  (unless (file-directory-p company-tabnine-binaries-folder)
...
(use-package counsel-itunes
  ...
  :bind
  (:prefix-map counsel-itunes-prefix-map
   :prefix "C-c i"
   ("-" . counsel-itunes-volume-down)
   ("+" . counsel-itunes-volume-up)
   ("t" . counsel-itunes-tracklist)
   ("P" . counsel-itunes-play-pause)
   ("p" . counsel-itunes-current-track)
                  ...
                  (let ((kk (apply #'ks k))
                        (desc (concat
                               (propertize icon
                                           'display '(raise -0.20))
                               " " rep)))
                    (which-key-add-key-based-replacements kk desc))))
    (wkr "" "iTunes")
    (wkr "" "volume down" "-")
    (wkr "" "volume up" "+")
    (wkr "" "tracklist" "t")
...
(use-package counsel-tramp
  :after (:all counsel term)
  :bind
  (:map terminal-prefix-map
           ("t" . counsel-tramp))
  :init
  (defun straight-recipes-installed (pkg)
    (let ((repo (symbol-name pkg)))
      (file-directory-p
       (resolve-path user-emacs-directory "straight" "build" repo))))

  (defalias #'package-installed-p #'straight-recipes-installed)

  :config
  (which-key-add-key-based-replacements
    "C-c t t" "Counsel Tramp"))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :tangle no
(use-package term
  :bind (:prefix-map terminal-prefix-map
         :prefix "C-c t")
  :init
  (defun term-send-tab ()
...
;; (when (display-graphic-p)
;;   (set-fontset-font "fontset-default" '#xF0C5
;;                     "github-octicons" nil 'prepend))

;; (which-key-add-key-based-replacements
;;   "M-m m w" '(" Web" . "Web Browsing"))
...
;; (set-fontset-font "fontset-default" '#xF02F "github-octicons" nil 'prepend)

;; (which-key-add-key-based-replacements "M-m w B"
;;   '(" Configure" . "Set the default browser" ))
...
(use-package engine-mode
  ...
  ;; :config
  ;; (when (eq system-type 'darwin)
  ;;   (set-fontset-font "fontset-default" '#xE600 "file-icons" nil 'prepend)
  ;;   (set-fontset-font "fontset-default" '#xE80C "Material Icons" nil 'prepend))
  ;;   (cl-flet* ((ks (&rest k)
...
(use-package lsp-java
  ...
  (lsp-java-server-install-dir
   (file-name-as-directory (emacs-var-dir "eclipse.jdt.ls" "server")))
...
#+BEGIN_SRC emacs-lisp :tangle no
  (use-package markdown-mode
    ...
    (markdown-command "markdown")
      ...
      ((markdown-mode gfm-mode) . org-visual-fill-column))
#+END_SRC
