...
(which-key-add-key-based-replacements "M-m s" '(" " . "straight.el"))

(define-key 'halidom-prefix-map (kbd "r") 'reload-eval-prefix-map)
(which-key-add-key-based-replacements "M-m r" '("" . "reload/eval"))
#+END_SRC
...
(use-package reveal-in-osx-finder
  ...
  :init
  (define-key 'macos-prefix-map (kbd "f") '(" Reveal in Finder" . reveal-in-osx-finder)))
#+END_SRC
...
(use-package osx-dictionary
  ...
  (progn
    ...
    (set-fontset-font "fontset-default" '#xE8CE "Material Icons" nil 'prepend)
    (which-key-add-key-based-replacements "M-m m d" '("" . "macOS Dictionary"))
...
(use-package counsel-osx-app
  ...
  :config
  (define-key 'macos-prefix-map "a" `(,(propertize "  " 'display '(raise -0.24)) .  counsel-osx-app)))
#+END_SRC
...
(when *is-mac*
  ...
  (set-fontset-font "fontset-default" '#xE917 "file-icons" nil 'prepend)
  (which-key-add-key-based-replacements "M-m o"
    `(,(propertize " Org Mode"
                   'font-lock-string-face
                   '(:family "file-icons") 'fontified nil) . "Org Mode")))
#+END_SRC
...
(use-package org-sidebar
  ...
  (cl-defun org-course-sidebar ()
    ...
    (let ((todo-file
           (car
            (directory-files (projectile-project-root) t "TODOs.org")))
          (sidebar-function
           (intern
            (concat
...
#+BEGIN_SRC emacs-lisp :tangle no
  (use-package org-projectile
    ...
    (defvar-local org-default-agenda-file (file-truename "~/Dropbox/org/todos/TODOS.org"))
...
theme in \"quick mode\" by eliding hooks specified in `halidom/load-theme-hook'."
...
(use-package pretty-eshell
  :if (file-exists-p
       (emacs-etc-dir "local/pretty-eshell/pretty-eshell.el"))
  :straight nil
...
(use-package counsel-itunes
  ...
  (define-prefix-command 'counsel-itunes-prefix-map)
   ...
   (let ((map counsel-itunes-prefix-map))
     ...
     (define-key map "p" '("Playlist" . counsel-itunes-play-pause))
  ...
  (which-key-add-key-based-replacements "M-m m i" '(" iTunes" . "iTunes")))
...
(which-key-add-key-based-replacements "M-m w"
  '(" Web" .  "Web Browsing"))
...
#+NAME: java-backend
#+BEGIN_SRC emacs-lisp :tangle no
(defcustom halidom-java-backend 'lsp
...
#+BEGIN_SRC emacs-lisp :tangle no
(defun java-setup ()
  "Setup the backend for `java-mode'."
  (require 'lsp-java)
  (lsp-java-enable)
  (company-mode 1)
