         ...
         (let ((pkg-directory
                (expand-file-name
                 (plist-get
                  ...
                  :local-repo)
                 dir)))
           (when (file-directory-p pkg-directory)
             (let ((readme (car
                            (directory-files
                             pkg-directory
                             t
...
#+BEGIN_SRC emacs-lisp :noweb yes :tangle no
  (use-package tex
    ...
    :config
    ;; Copied from Emacs Stack exchange
    ;; https://emacs.stackexchange.com/a/5493
...
#+BEGIN_SRC emacs-lisp :tangle no
  (defun org-preview-process-hook ()
    ...
    (let ((latex-compiler (org-preview/process-compiler))
          (foreground-color (face-attribute
...
#+BEGIN_SRC emacs-lisp :tangle no
  ...
  (which-key-add-prefix-title "M-m o" "Org Mode")
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :tangle no
  (use-package org-outline-numbering
    ...
    :hook
    (org-mode . org/outline-numbering))
...
*** Highlight inline LaTeX fragments :disabled:
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (use-package skeletor
   ...
   `skeletor-python-bin-search-path'."
    ...
    (defun skeletor/redact-user-directory (pkg xs)
      "When a pkg PKG is available,
  add `skeletor-directory' to a list XS so that
  templates are not marked as projects."
      (when (and (featurep pkg)
               (boundp xs))
        (add-to-list xs skeletor-user-directory)))

    (defun skeletor/lsp-blacklist ()
      (skeletor/redact-user-directory
       'lsp-mode
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (use-package dired-sidebar
      ...
      :custom (dired-sidebar-should-follow-file t)
      :init
      (defun sidebar/disable-icons ()
        (when (fboundp 'all-the-icons-dired-mode)
          (set
           (make-local-variable 'all-the-icons-dired-mode)
           nil)))
      :hook
      (dired-sidebar-mode . sidebar/disable-icons)
      :bind
...
#+BEGIN_SRC emacs-lisp :tangle no
  ...
  package initialization time and garbage collections."
    ...
    (defun goto-dashboard ()
      "Switch to the `dashboard-buffer-name'buffer."
      (interactive)
      (if (seq-contains
    ...
    (defun dashboard/set-initial-buffer ()
      (when (and (boundp 'dashboard-buffer-name)
               (bufferp (get-buffer dashboard-buffer-name)))
    ...
    (setq initial-buffer-choice (dashboard/set-initial-buffer))
           ...
           (theme-name (if (interactive-p)
                         ...
                         (eval (intern
                                (concat
                                 "halidom-"
                                 (format "%s"
                                         (symbol-value 'halidom--theme-style))
                                 "-theme")))))
         (theme-symbol (if (symbolp theme-name) theme-name (intern theme-name))))
      (remove-themes)
...
#+BEGIN_SRC emacs-lisp :tangle no
  ...
  ;; Org Mode faces
    ...
    (defun halidom/disable-scroll-bar ()
      (toggle-scroll-bar -1))
...
#+BEGIN_SRC emacs-lisp :tangle no
  ...
  (defun halidom/font-setup ()
    ...
    (let* ((font-family
            (or
             (car (seq-intersection (mapcar #'car halidom-fonts) (font-family-list)))
...
This mode actually degrades the quality of emoticons on macOS.
...
(setq org-fixed-pitch-font "Operator Mono")
