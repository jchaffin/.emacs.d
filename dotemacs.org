...
**** Lentic
:PROPERTIES:
:ID:       94A76A4D-A77C-4D43-A4D8-C384BA571FC2
:END:
#+BEGIN_SRC emacs-lisp
(use-package lentic)
#+END_SRC
...
#+NAME: fd-read-only
#+BEGIN_SRC emacs-lisp
(defun read-only-directory-hook ()
  (dolist (pattern '("~/.emacs.d/straight"
                               "/usr/local/Cellar"))
    (when (and buffer-file-name
               (string-match (expand-file-name pattern) buffer-file-name))
      (read-only-mode))))

(add-hook #'find-file-hook 'read-only-directory-hook)
#+END_Src
...
#+BEGIN_SRC emacs-lisp
(defun counsel-straight-installed-packages ()
...
(defun straight-installed-p (pkg)
  (member (symbol-name pkg) (counsel-straight-installed-packages)))


(defun counsel-straight-browse-local-strategy (build-dir dir package)
...
(cl-defun counsel-straight-browse-local (&optional build-dir)
  ...
  (lexical-let* ((dir (-> user-emacs-directory
                 ...
                 (pkg-keys (counsel-straight-installed-packages)))
    (ivy-read
     msg
     pkg-keys
     :action
     (lambda (package)
       (counsel-straight-browse-local-strategy build-dir dir package)))))

(define-key goto-map "r" #'counsel-straight-browse-local)
#+END_SRC

*** Browse straight repository at GitHub

#+BEGIN_SRC emacs-lisp
(defun counsel-straight-browse-remote (&optional package)
  ...
  (cl-flet ((remote-url (pkg)
                  ...
                  (cond ((eq host 'github) (concat "https://github.com/" repo))
                        ...
                        (t (message "Cannot get remote url for %s" pkg)))))))
    (let* ((pkg (if (interactive-p)
                    (completing-read
                     "Which recipe? "
                     (straight-recipes-list straight-recipe-repositories)
                     nil 'require-match)))
           (url (remote-url (intern (or pkg package)))))
      (browse-url url))))
#+END_SRC
...
(defun straight-browse-remote-action (pkg)
  ...
  (counsel-straight-browse-remote pkg))

(defun counsel-straight-browse-local-action (pkg)
  "ivy action to browse installed recipe. "
  (let ((repo-dir (expand-file-name "straight/repos" user-emacs-directory)))
    (counsel-straight-browse-local-strategy nil repo-dir pkg)))


(with-eval-after-load 'ivy
  (ivy-set-actions 'straight-use-package
                   '(("b" counsel-straight-browse-local-action "browse local")
                     ("B" counsel-straight-browse-remote-action "browse remote"))))
#+END_SRC
...
(use-package hydra
  ...
  "
      ("b" counsel-straight-browse-local)
      ("B" counsel-straight-browse-remote)
...
value of `user-emacs-directory', else go to the`user-init-file'."
  (interactive)
  (let ((init-file
         (if (file-exists-p (expand-file-name ".emacs-profiles.el" "~"))
             (expand-file-name "init.el" user-emacs-directory)
           user-init-file)))
    (find-file init-file)))

(defun goto-literate (&optional arg)
  "Open the literate Emacs config file."
  (interactive)
  (let ((file dotemacs-user-literate-init-file))
...
[[https://github.com/abo-abo/swiper][Ivy]] is a completion and selection framework in the same vein as helm.

It doesn't have the same kind of ecosystem or interopability, but its easy to configure, offers a minimalistic interface, and is every bit as good of a completion tool as helm is, if not better.
...
(use-package org-player
  :straight nil
  :load-path "etc/local/org-player"
  :init
  (require 'org-player))
#+END_SRC
  ...
  (use-package org-projectile
    ...
    (defun org-projectile-per-project-function (project-path)
      (let  ((project-agenda-file
              (--> project-path (split-string it "/" t) nreverse car
                 (concat it ".org"))))
...
Magit now uses  [[https://emacsair.me/2019/02/14/transient-0.1][transient]], but some packages still use =magit-popup= as a dependency.
