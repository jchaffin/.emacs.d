...
(use-package tramp
  ...
  (when (boundp 'tramp-remote-path)
    (add-to-list "/usr/local/cs/bin" tramp-remote-path))
...
;; https://github.com/syl20bnr/spacemacs/pull/7667
  (defun org-modified-theme  (&optional skip-config-p)
    "Restart all open org-mode buffers."
    (interactive "P")
    (let* ((org-buffers (org-buffer-list  'files t))
           (agenda-buffers (org-buffer-list 'agenda t))
           (config-buffer (get-file-buffer dotemacs-literate-config-file)))
      (org-theme-faces)
      (dolist (buf org-buffers)
        (unless (and skip-config-p (equal buf config-buffer))
          (with-current-buffer buf
            (org-mode-restart)
            (when (member buf (seq-intersection org-buffers agenda-buffers))
              (org-agenda-theme)))))))
...
:hook
(load-theme . org-modified-theme)
((load-theme after-make-frame-functions) . disable-scroll-bar))
