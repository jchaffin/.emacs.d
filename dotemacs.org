...
;; https://github.com/syl20bnr/spacemacs/pull/7667
  (defun org-mode-reset (&optional skip-config-p)
    "Restart all open org-mode buffers."
    (let ((org-buffers (org-buffer-list  'files t))
)
      (dolist (buf org-buffers)
        (with-current-buffer buf
          (unless (and skip-config-p
                       (string=
                        (buffer-name buf)
                        (basename dotemacs-literate-config-file)))
          (org-mode-restart))))))
