...
(with-eval-after-load 'ivy
  (ivy-set-actions 'straight-use-package
                   '(("d" straight-browse-local "browse local")
                     ("h" straight-browse-remote "browse remote"))))
#+END_SRC
...
(use-package org-ref
  ...
  (defun org-ref-algref-key-under-cursor ()
...
properties."
            ...
            (let (link-string-beginning link-string-end)
              ...
              (let (key-beginning key-end)
                ...
                (let ((ref-key
                  ...
                  (set-text-properties 0 (length ref-key) nil ref-key) ref-key)))))))


  (defun org-ref-algref-message ()
    (let ((label (org-ref-algref-key-under-cursor)))
      (if (= (org-ref-count-labels label) 0)
          (message "%s Context not found ! count: 0")
        (let ((context (org-ref-get-label-context label))
              (count          (org-ref-count-labels label)))
          (set-text-properties 0 (length context) nil context)
          (message "%scount:%s" context count)))))


  (defun org-ref-algref-help-echo (window object position)
    "A help-echo function for ref links."
    (save-excursion
      (goto-char position)
      (let ((s (org-ref-algref-message)))
        (with-temp-buffer
	        (insert s)
	        (fill-paragraph)
	        (buffer-string)))))

  (defun org-ref-algref-export (keyword desc format)
  ...
  (org-ref-link-set-parameters "algref"
    :follow (lambda (_) (funcall #'org-ref-ref-follow (org-ref-algref-key-under-cursor)))
    :export #'org-ref-algref-export
    :complete #'org-ref-algref-complete
    :face (lambda (_) (funcall #'org-ref-ref-face-fn (org-ref-algref-key-under-cursor)))
    :help-echo #'org-ref-algref-help-echo)
  ...
  (use-package org-agenda
    ...
    (setq org-agenda-category-icon-alist
     `(("global"     ,(list "")  nil nil :ascent center)
       ("semantics"  ,(list "𝜆")  nil nil :ascent center)
       ("phonetics"  ,(list "ə")  nil nil :ascent center)
       ("AI"         ,(list "∞") nil nil :ascent center)
       ("languages"  ,(list "") nil nil :ascent center)))
    ...
    (defun org-projectile-per-project-function (project-path)
      (let  ((project-agenda-file
              (--> project-path
                 ...
                 (concat it ".org"))))
        (if (file-exists-p
             (expand-file-name project-agenda-file project-path))
