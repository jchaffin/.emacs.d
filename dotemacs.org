...
(defun dotemacs/open-buffer-as-read-only (file)
...
(defun dotemacs/find-file-read-only-hook ()
  (funcall #'dotemacs/open-buffer-as-read-only (buffer-file-name)))

(setq read-only-directories `(,(straight-dir :slash t) "/usr/local/"))

(setq read-only-whitelist-directories '("/usr/local/src/llvm-project/"))
(add-hook 'find-file-hook #'dotemacs/find-file-read-only-hook)
#+END_SRC
...
prompting."
    ...
    (setq org-capture-templates
          '(,("C" "Config"
                  (file+function
                   ,dotemacs-user-literate-init-file
...
#+BEGIN_SRC emacs-lisp
  (use-package org-agenda
    ...
    (defun dotemacs/org-agenda-file-face ()
    ...
    (defvar dotemacs/default-agenda-file "~/Dropbox/org/todos/TODOs.org"
    ...
    :hook
    (org-mode . dotemacs/org-agenda-file-face))
#+END_SRC
...
(use-package base16-theme
    ...
    (setq dotemacs-dark-theme 'base16-snazzy
     ...
     (defun dotemacs--base16-p (&optional theme)
 ...
 `dotemacs/load-theme-hook'."
  ...
  (let ((theme-symbol (if (symbolp theme) theme (intern theme))))
    ...
    (if (not window-system)
      ...
      (if (dotemacs--base16-p dotemacs-theme)
          (setq dotemacs-theme-colors
                (symbol-value
                 (intern
                  (eval
                   `(concat
                     ,(symbol-name (symbol-value 'dotemacs-theme))
                     "-colors")))))
