...
(use-package macos-dev-utils
  ...
  :bind (:map macos-prefix-map
              ("t" . macos-iterm-command-map)
              ("o" . macos-open-with-command-map))
  :config
...
(setq read-only-whitelist '("/usr/local/src/llvm-project"))
...
(defun halidom/font-setup (&optional frame)
  (let* ((font- (or (car
                    (seq-intersection halidom-fonts (font-family-list)))
                   (face-attribute 'fixed-pitch :family)))
         (font (concat font- "-" (format "%s" halidom-macos-default-font-size))))
...
(defun halidom/toggle-theme ()
  ...
  (halidom/load-theme))
(defcustom halidom-italicize-keyword-modes '(emacs-lisp-mode js-mode)
...
(use-package company-box
    ...
    :hook (company-mode . company-box-mode)x)
...
(use-package counsel-itunes
  ...
  :init
   ...
   (define-key 'halidom-prefix-map "i" 'counsel-itunes-keymap)
   ;; define keymap
...
(use-package cquery
    ...
    :config
      ...
      (add-hook 'c-mode-hook #'c-cxx/setup-flycheck)
      (add-hook 'c++-mode-hook #'c-cxx/setup-flycheck)
...
(use-package lsp-clangd
    :straight (lsp-clangd
               ...
               :host github)
    :commands (lsp-clangd-c-enable lsp-clangd-c++-enable lsp-clangd-objc-enable)
    ...
    :init
    (when *is-mac*
      (setq lsp-clangd-executable (executable-find "clangd")))
...
(use-package flycheck-clang-tidy
    ...
    (defun clang-tidy//enable ()
      (when (and (-non-nil projectile-project-root)
                 (file-exists-p (f-join (or projectile-project-root default-directory)
                                        "build"
...
(use-package js2-mode
  ...
  :hook ((js2-mode . js-setup-backend)
         (js2-mode . halidom/flycheck-enable-hook)))
#+END_SRC
