...
(use-package spaceline-all-the-icons
  ...
  (progn 
    ...
    (defun battery-macos ()
      (let* ((battery-shell-command  "pmset -g batt | egrep \"([0-9]+\%).*\" -o --colour=auto | cut -f1 -d';'")
	     (battery-string
	      (replace-regexp-in-string "\n" ""
					(shell-command-to-string
					 battery-shell-command))))
	(string-to-number battery-string)))

    (defun battery-macos-spaceline-msg (icon percentage)
      (concat (number-to-string percentage) "%% " icon))

    (spaceline-define-segment battery
      (let ((battery-percentage (battery-macos)))
	(cond ((= battery-percentage 100)
	       (battery-macos-spaceline-msg "" battery-percentage))
	      ((> battery-percentage 75)
	       (battery-macos-spaceline-msg "" battery-percentage))
	      ((> battery-percentage 50)
	       (battery-macos-spaeline-msg "" battery-percentage))
	      ((> battery-percentage 25 "" battery-percentage))
	      ((> battery-percentage 0 "" battery-percentage)))))

    (spaceline-all-the-icons-theme 'org-pomodoro 'battery)
