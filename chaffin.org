...
#+BEGIN_SRC emacs-lisp :tangle no
(defvar read-only-directories '("/Users/jacobchaffin/.emacs.d/straight/"))
(defvar read-only-file-extensions '("\\.gz\\'"))
(defvar user-home-directory "/Users/jacobchaffin/")
...
(defun chaffin//dir-or-subdir-p (d1 d2)
  (cond ((= (dir-level d1) (dir-level d2)) nil)
        ((string= d1 d2) t)
        (t (chaffin//dir-or-subdir-p (file-name-directory (directory-file-name d1)) d2))))

(defun chaffin//open-buffer-as-read-only-p (bf)
...
(defun chaffin/open-buffer-as-read-only (file)
  "All buffers from `read-only-directories' or `read-only-file-extensions' are set to read-only."
  (let ((bd (and buffer-file-name (file-name-directory file))))
    (if (chaffin//open-buffer-as-read-only-p bd) (read-only-mode 1))))
