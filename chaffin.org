...
(when (fboundp 'chaffin-keymap)
  (define-key #'chaffin-keymap "g" #'straight-browse-at-github))
#+END_SRC
...
**** Webkit Katex Renderer

#+NAME: latex-preview-katex
#+BEGIN_SRC emacs-lisp :tangle no
(use-package webkit-katex-render
  :straight (webkit-katex-render :host github
                                 :repo "fuxialexander/emacs-webkit-katex-render"
                                 :files ("katex.html" "webkit-katex-render.el"))
  :demand t
  :init
...
(setq org-format-lualatex-header "\\documentclass{article}
...
(add-hook 'org-mode-hook #'org-preview-luamagick-fragments)

(defun org-preview-clear-cache ()
  (interactive)
  (let ((preview-cache (f-join default-directory "ltximg")))
    (if (f-directory? preview-cache)
        (f-delete preview-cache t)
      (message "%s" "Directory 'ltximg' does not exist."))))

(defun org-preview-algorithm--bg ()
  (substring
   (apply #'color-rgb-to-hex 
          (color-complement (frame-parameter nil 'background-color))) 0 7))
      ...
      (lambda (env)
        (let ((beg (org-element-property :begin env))
          ...
          (if (string-match "\\\\begin{algorithm}" val)
              (setq algop t)))))
    (when algop
      (setq options (plist-put options :background (org-preview-algorithm--bg))))
    (setf (nth 2 args) options)
    (apply orig-func args)))

(advice-add 'org-create-formula-image :around #'org-preview-algorithm-environment)
;; (advice-remove 'org-create-formula-image #'org-preview-algorithm-environment)
#+END_SRC
...
(defun chaffin--org-mode-vfc-hook ()
  (visual-fill-column-mode 1)
  (visual-line-mode 1))

(add-hook 'org-mode-hook #'chaffin--org-mode-vfc-hook)
...
(use-package org-sidebar
  :after (:all org-habit org-agenda-ng org-ql)
  :demand t
...
(defvar org-latex-pdf-process-set-hook )
#+END_SRC
...
(use-package projectile
  ...
  (progn
    ...
    (defvar chaffin/ignored-project-directories '("~/.emacs.d/straight"))
    (defun projectile-ignore-projects-in-directory (project-root)
      (cl-flet ((ignored-dir-or-subdir-p (path) (f-descendant-of? (f-expand project-root) path)))
        (->> chaffin/ignored-project-directories (seq-filter #'ignored-dir-or-subdir-p) seq-empty-p not)))
    (setq projectile-ignored-project-function #'projectile-ignore-projects-in-directory)))
#+END_SRC
...
(defun chaffin/find-file-read-only-hook ()
  (funcall #'chaffin/open-buffer-as-read-only (buffer-file-name)))

(setq read-only-directories '("/Users/jacobchaffin/.emacs.d/straight/"
                              "/usr/local/Cellar/emacs-plus/"))

(add-hook 'find-file-hook #'chaffin/find-file-read-only-hook)
...
(use-package js2-mode
  ...
  (progn
    ...
    (when *is-mac*
      (defun macos-dash-js-docsets ()
        (set (make-local-variable 'dash-plugin-keywords)
             (if *is-mac*
               '("javascript" "react" "jquery"
                 ...
                 "webpack"))))
      (add-hook 'js2-mode-hook #'macos-dash-js-docsets))))
#+END_SRC
