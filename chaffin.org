...
(use-package spaceline-all-the-icons
  ...
  :config
  <<spaceline-segment>>
  (spaceline-all-the-icons-theme 'org-pomodoro 'battery-macos))
#+END_SRC
...
;; Macros
(defmacro battery-macos/if-args (arg-list &rest body)
  `(if ,arg-list
       ,@body))
...
(defun battery-macos-status-icon-propertize (battery-icon-var)
  (let* ((battery-icon-name (car battery-icon-var))
    ...
    (propertize battery-icon-unicode
		...
		'display '(raise 0.10)
		'face
		`(:inherit (powerline-active2) :family ,battery-icon-font-family)
		'height 1.2)))
