...
#+BEGIN_SRC http :pretty :file ./documentation-is-for-wimps.png
...
#+RESULTS:
[[file:./documentation-is-for-wimps.png]]
  ...
  (let* ((ms-file-path-separator "\\")
         ...
         (file-path-separator
          (if (eq system-name 'windows)
            ...
            unixy-file-path-separator)))
    (mapconcat 'identity path file-path-separator)))
#+END_SRC
...
Function to jump to a repository installed by =straight.  an interactive minibuffer completion menu using ivy  
...
I work on a macbook, so this block is where I'm loading all of my
settings that rely on local filepaths, macOS applications, and
external programs.
...
(use-package org-pomodoro
  ...
  (setq org-pomodoro-audio-player "afplay"
        ...
        org-pomodoro-start-sound (org-pomodoro-path "Waves.wav")
...
(defalias #'org-pomodoro-path
  (apply-partially #'emacs-directory-resolve-path "resources"))
#+END_SRC
...
(when *is-mac*
  (add-to-list-proglang-enabled 'c-c++ 'common-lisp 'clojure 'groovy 'java
