...
- [[#configuration][Configuration]]
  ...
  - [[#org-mode][Org Mode]]
    ...
    - [[#org-latex][Org LaTeX]]
      ...
      - [[#org-ref][Org Ref]]
      - [[#org-pdf][Org PDF]]
      - [[#org-bibtex-extras][Org BibTeX Extras]]
...
Tools for checking lexical and grammatical errors in text buffers[fn:9].
...
***** Adjust Hour of Task Deadline

From [[https://emacs.stackexchange.com/a/14441][@14441]] on Emacs Stack Exchange:

#+NAME: org-agenda-date-hour-deadline
#+BEGIN_SRC emacs-lisp :tangle no
(require 'cl-lib)

(defvar org-deadline-default-time "23:59"
...
(defun org-ref-vref-export (keyword desc format)
  (cond
   ...
   ((eq format 'html) (format "\\vref{%s}" keyword))
   ((eq format 'md)
...
****** Org Ref Bibtex Citation Formats

#+NAME: ore-orcb
#+BEGIN_SRC emacs-lisp :tangle no :noweb yes :exports none
<<orcb-year-author-doi>>
(add-to-list 'org-ref-clean-bibtex-entry-hook 'orcb-year-author-doi)
#+END_SRC

******* Year-Author-DOI

#+NAME: orcb-year-author-doi
#+BEGIN_SRC emacs-lisp :tangle no
...
******* Pandoc Citations                                       :ignore:

See [[http://kitchingroup.cheme.cmu.edu/blog/2015/01/29/Export-org-mode-to-docx-with-citations-via-pandoc/][kitchingroup]] article.
...
(defalias #'ivy-bibtex-format-pandoc-citation #'helm-bibtex-format-pandoc-citation)

;; inform completion framework (helm/ivy) how to format the citation in org-mde
(setf (cdr (assoc 'org-mode bibtex-completion-format-citation-functions))
      'ivy-bibtex-format-pandoc-citation)
...
(defun ox-export-to-docx-and-open ()
 ...
 (let* ((bibfile (expand-file-name (car (org-ref-find-bibliography))))
   ...
   (shell-command (format
                   "pandoc -s -smart --bibliography=%s %s -o %s"
...
(setq orbj-extra-abbrevs
      '(("JPS" "Journal of Palestinian Studies")
        ...
        ("MEastStudies" "International Journal of Middle East Studies")))
...
#+NAME: org-latex-bibtex-extras
#+BEGIN_SRC emacs-lisp :tangle no 
(use-package org-bibtex-extras
  :if *is-mac*
  :straight nil
  :after (org)
  :demand t
  :config
  (progn
    (setq obe-bibtex-file "~/Dropbox/org/references.org")))
#+END_SRC
...
Tabs are the bane of humanity[fn:13]. [[http://www.urbandictionary.com/define.php?term=dont%20%40%20me][Don't @ me]].
...
[fn:9] https://joelkuiper.eu/spellcheck_emacs

[fn:10] See [[https://matt.hackinghistory.ca/2015/11/11/note-taking-with-pdf-tools/][Note taking with pdf-tools]]

[fn:11] [[https://www.masteringemacs.org/article/keeping-secrets-in-emacs-gnupg-auth-sources][Mastering Emacs - Keeping Secrets in Emacs GnuPG Auth Sources]]

[fn:12] https://www.emacswiki.org/emacs/NeoTree

[fn:13] [[https://www.emacswiki.org/emacs/TabsAreEvil][Emacs Wiki - Tabs Are Evil]]

[fn:14] [[http://fgiasson.com/blog/index.php/2016/06/14/my-optimal-gnu-emacs-settings-for-developing-clojure-revised/][fgiasson.com - My Optimal Gnu Emacs Settings for Developing
Clojure Revised]]

[fn:15] [[https://www.gnu.org/software/emacs/manual/html_node/elisp/Library-Headers.html#Library-Headers][Emacs Manual - D.8 Conventional Headers for Emacs Libraries]]
