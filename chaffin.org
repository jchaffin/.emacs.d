...
(defun copy-file-path-as-region-kill (file-path)
  "Copy file-path using a temporary buffer."
  (interactive)
  (copy-file-path 
    (lambda (file-path)
		    (with-temp-buffer
...
(defvar minor-modes-enabled-list (list-enabled-minor-modes)
...
(defvar halidom-default-theme 'base16-oceanicnext)
(defvar halidom-term-theme 'zenburn)

(setq-default custom-enabled-themes '(base16-oceanicnext))

(defun remove-themes ()
  "Forcibly load the themes listed in `custom-enabled-themes'."
  (mapc #'disable-theme custom-enabled-themes))

(defvar chaffin/load-theme-hook nil)
...
(defun chaffin/load-theme ()
  ...
  (if (display-graphic-p)
      (progn
        ...
        (run-hooks 'chaffin/load-theme-hook))

    (progn
      (load-theme halidom-term-theme))))


(defun chaffin-theme-load-update-buffer ()
  (auto-revert-mode)
  (setq-local buffer-stale-function
              #'(lambda (&optional noconfirm) 'fast)))

(add-hook 'chaffin/load-theme-hook #'chaffin-theme-load-update-buffer)

(add-hook 'after-init-hook #'chaffin/load-theme)
