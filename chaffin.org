...
(when (and (executable-find "gpg") *is-mac*)
  (if-not (string-empty-p
      ...
      (load-library (emacs-dir "secrets" "secrets.el.gpg"))
...
(let ((map reload-eval-prefix-map))
  ...
  (define-key map "r" 'straight-eval-region)
...
(use-package macos-dev-utils
  ...
  :defines (macos-iterm-command-map macos-open-with-command-map)
  :config
...
(use-package async
  ...
  :custom (message-send-mail-function 'async-smtpmail-send-it)
  :init
  (dired-async-mode 1))
#+END_SRC
...
(use-package lsp-mode
  ...
  (lsp-render-markdown-content 'gfm-view-mode)
...
(use-package json-mode
  ...
  (add-hook 'json-mode-hook #'json-mode-faces)
  (add-hook 'json-mode-hook 'json-mode-style))
...
(use-package tide
    ...
    (defun setup-tide-mode ()
      ...
      ((typescript-mode-hook . setup-tide-mode)
       ...
       (typescript-mode-hook . js-setup-backend))))
#+END_SRC
